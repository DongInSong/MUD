...
namespace mud
{
    file::file(boost::asio::ip::tcp::socket& socket)
        : socket_(socket)
    {
    }
    ...
    void file::send_file(const std::string& file_path, std::function<void(bool)> callback)
    {
        ...
        boost::asio::async_write(socket_, boost::asio::buffer(header),
            [this, callback](const boost::system::error_code& error, std::size_t /*bytes_transferred*/) {
                if (!error) {
                    ...
                    std::cerr << "Send file header error: " << error.message() << std::endl;
                    callback(false);
                }
            });
    }

    void file::receive_file(const std::string& file_name, std::size_t file_size, std::function<void(bool)> callback)
    {
            ...
            boost::asio::async_write(socket_, boost::asio::buffer(buffer_.data(), bytes_read),
                [this, callback](const boost::system::error_code& error, std::size_t /*bytes_transferred*/) {
                    if (!error) {
                        ...
                        std::cerr << "Send file content error: " << error.message() << std::endl;
                        callback(false);
                    }
                });
        } else {
    ...
    void file::do_read_file_content(std::size_t remaining_size, std::function<void(bool)> callback)
    {
        if (remaining_size == 0) {
            callback(true);
            return;
        ...
        socket_.async_read_some(boost::asio::buffer(buffer_),
            [this, remaining_size, callback](const boost::system::error_code& error, std::size_t bytes_transferred) {
                if (!error) {
                    output_file_.write(buffer_.data(), bytes_transferred);
                    
                    std::cout << "\rDownloading... " 
                              << std::fixed << std::setprecision(2) 
                              << (double)(remaining_size - bytes_transferred) / remaining_size * 100 << "%" << std::flush;

                    do_read_file_content(remaining_size - bytes_transferred, callback);
                } else {
                    std::cerr << "Receive file content error: " << error.message() << std::endl;
                    callback(false);
                }
            });
    }
